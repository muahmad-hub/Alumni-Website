...
from channels.db import database_sync_to_async  # To handle sync DB calls in async

class MessageConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        self.user = self.scope['user']
        self.group_name = self.scope['url_route']['kwargs']['group_name']
        # Use await because get_object_or_404 is sync
    ...
    async def receive(self, text_data):
        ...
        # Save message via sync DB call wrapped for async
        ...
        # Rendering template is sync, wrap it too
