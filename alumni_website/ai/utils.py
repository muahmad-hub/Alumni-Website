...
def get_model_and_tokenizer():
    global tfidf_vectorizer, embedding_mapper, metadata
    
    # Retrieves the loaded components

    if tfidf_vectorizer is None:
        tfidf_vectorizer = joblib.load("ai/models/tfidf_vectorizer.pkl")
        embedding_mapper = joblib.load("ai/models/embedding_mapper.pkl")
        
    return tfidf_vectorizer, embedding_mapper

def vectorize(text):
    tfidf_vectorizer, embedding_mapper = get_model_and_tokenizer()
    
    # Generates the TF-IDF vector
    tf_idf_vector = tfidf_vectorizer.transform([text])
    
    # Predicts what the BERT embedding would be
    vector = embedding_mapper.predict(tf_idf_vector)[0]
